...
void setup()
{
  ...
  Serial.begin(9600);
  //was testing out to see in serial monitor
  Serial.println(NOTE_C5);
  Serial.println(NOTE_D5);
...
void record()
{
  ...
  if (sound > 0) {
    //Convert the sound to a pitch
    int tempSound = convertSound(sound);
    //play the sound that was pressed and save it.
    Serial.println(sound);
    tone(BUZZER_PIN, tempSound, duration);

    //If the max is not reached for notes, save the note.
    if (countNotes < MAX_NOTES) {
      notes[countNotes] = tempSound;
...
int convertSound(int sound){
//  int pitches[] = {NOTE_C5, NOTE_D5, NOTE_E5, NOTE_F5, NOTE_G5};
//  int soundConstant[] = {7, 83, 487, 931, 1023};
// in each range there's a note for it to be played.
// when the buttons reach to its capacity, it'll play the note
  if(sound < 75){
    return NOTE_C5;
  }
  else if(sound >=75 && sound < 480){
    return NOTE_D5;
  }
  else if(sound >= 480 && sound < 925){
    return NOTE_E5;
  }
  else if(sound >=925 && sound < 1015){
    return NOTE_F5;
  }
  else if(sound >= 1015){
    return NOTE_G5;
