...
void live()
{
   ...
   //value of buttons
   int pinVal = analogRead(NOTE_IN_PIN);

   //if pinVal is greater than noise when button is pressed
   if (pinVal > theAdjuster) {
    //play tone
    tone(BUZZER_PIN, pinVal + offsetFrequency, 100);
   }
...
void playCurrentNote()
{
    ...
    //set current tone to active frequency
...
int getPhotoFrequency()
{
  ...
  //return to photocell value
  return map(photoVal, 0, 1023, 0, 255);
}
...
int getRunningAverage()
{
 ...
 if (nextCount < RUNNING_SAMPLES) {
  //fill the running average buffer
 ...
 //sum of photocell's input
 int totalSum = 0;

 //loop to calculate sum
 for (int i = 0; i <RUNNING_SAMPLES; i++) {
  //adding sum
  totalSum += runningAverageBuffer[i];
 }

 //return to average
 return totalSum / RUNNING_SAMPLES;
...
{
  //check to not exceed limit size
  if (countNotes < MAX_NOTES) {
    tone(BUZZER_PIN, notes[countNotes], durations[countNotes]);
    //change light intensity
    colorLED(map(notes[countNotes], 0, 1023, 0, 255));
    //delay
